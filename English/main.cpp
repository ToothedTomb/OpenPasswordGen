...
void on_generate_password(GtkWidget *widget, gpointer data) {
    ...
    try {
        ...
        gtk_entry_set_text(GTK_ENTRY(entry_password), "Error 1 - Invalid length");
    } catch (std::out_of_range&) {
        gtk_entry_set_text(GTK_ENTRY(entry_password), "Error 2 - Length out of range");
    }
}
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Open Password Generator!");
    ...
    // Title
    GtkWidget *title = gtk_label_new("<u><b><big> Open Password Generator: </big></b></u>");
    ...
    GtkWidget *label_length = gtk_label_new("Enter password length:");
    gtk_box_pack_start(GTK_BOX(vbox), label_length, FALSE, FALSE, 5);

    entry_length = gtk_entry_new();
    gtk_entry_set_placeholder_text(GTK_ENTRY(entry_length), "Example - 12");
    ...
    gtk_entry_set_placeholder_text(GTK_ENTRY(entry_password), "Generated password will appear here");
